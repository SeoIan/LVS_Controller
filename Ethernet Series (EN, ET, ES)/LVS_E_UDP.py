...
class LVSEthernet:
    def __init__(self, host = '127.0.0.1', port = 8080):
        self._communicator : socket.socket = None
        self._buffer_size : int = 1024
        self._host : str = host
        self._port : int = port
        self._remote_addr : tuple[str, int] = (host, port)
        self._worker : threading.Thread = None
        self._stop_event : threading.Event = threading.Event()
        self._lock : threading.Lock = threading.Lock()

    def open(self):
        self._communicator = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._communicator.setblocking(False)
    ...
    def send(self, message : bytes):
        if not isinstance(message, bytes):
            return
        self._communicator.sendto(message, self._remote_addr)
    ...
    def receive(self) -> bytes:
        ...
        try:
            data, _ = self._communicator.recvfrom(self._buffer_size)
            return data
...
if __name__ == "__main__":
    communicator = LVSEthernet(host="192.168.0.230", port=50000)
    communicator.open()
    CMD_WRITE_SVR = SOH + OPCODE_WRITE + Data_Length_2 + REG_SVR + communicator.int2bytes(130, 2) + EOT
    
    communicator.send(CMD_WRITE_SVR)
    time.sleep(1) 
    
    CMD_READ_SVR = SOH + OPCODE_READ + Data_Length_2 + REG_SVR + EOT
    communicator.send(CMD_READ_SVR)
    time.sleep(1) 
